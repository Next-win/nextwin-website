    ...
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md' : 'bg-transparent'}`}>
      ...
      {isMobileMenuOpen && (
        <motion.div 
          ...
          className={`md:hidden ${mobileBgClass} shadow-lg py-4 max-h-[80vh] overflow-y-auto`}
        >
                ...
                link.hasDropdown ? (
                  <div key={link.name}>
                    <button
                      ...
                      className={`${mobileTextClass} ${mobileHoverClass} font-medium transition-colors py-2 flex items-center justify-between w-full`}
                    >
                      {link.name}
                      <svg 
                        className={`ml-1 h-4 w-4 transition-transform duration-300 ${isServicesDropdownOpen ? 'rotate-180' : ''}`} 
                    ...
                    {isServicesDropdownOpen && (
                      <div className={`pl-4 border-l-2 ${isScrolled ? 'border-gray-200' : 'border-gray-700'} mt-2 mb-4`}>
                        <Link 
                          href="/services"
                          className={`flex items-center py-3 ${mobileTextClass} ${mobileHoverClass} transition-colors`}
                          onClick={() => {
                            setIsServicesDropdownOpen(false);
                        ...
                        {serviceLinks.slice(1).map((category, idx) => (
                          <div key={idx} className="my-3">
                            {isServiceCategory(category) && (
                              <>
                                <h4 className={`text-xs font-semibold ${isScrolled ? 'text-gray-500' : 'text-gray-400'} uppercase tracking-wider mb-2`}>
                                  {category.category}
                                </h4>
                                <div className="space-y-3">
                                  {category.items.map((item) => (
                                    <Link 
                                      key={item.name} 
                                      href={item.href}
                                      className={`flex items-center py-2 ${mobileTextClass} ${mobileHoverClass} transition-colors group`}
                                      onClick={() => {
                                        setIsServicesDropdownOpen(false);
                                        setIsMobileMenuOpen(false);
                                      }}
                  ...
                  <Link 
                    key={link.name} 
                    href={link.href}
                    className={`${mobileTextClass} ${mobileHoverClass} font-medium transition-colors py-2`}
