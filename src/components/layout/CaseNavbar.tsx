...
const CaseNavbar = () => {
  ...
  const dropdownClass = isScrolled ? 'text-gray-700' : 'text-white';
  const mobileBgClass = isScrolled ? 'bg-white' : 'bg-gray-900';
  const mobileTextClass = isScrolled ? 'text-gray-700' : 'text-gray-300';
  const mobileHoverClass = isScrolled ? 'hover:text-primary-700' : 'hover:text-white';
  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md' : 'bg-transparent'}`}>
        ...
        <div className="flex justify-between items-center py-4">
          ...
          <Link href="/" className="flex items-center">
            <Image 
              ...
              width={150} 
              height={28} 
              className={`h-7 w-auto ${logoClass}`}
            />
          </Link>
          ...
          <nav className="hidden md:flex space-x-8">
            {navLinks.map((link) => (
              link.hasDropdown ? (
                ...
                >
                  <button
                    className={`${textClass} hover:text-primary-700 font-medium transition-colors flex items-center`}
                  >
                    {link.name}
                    <svg 
                ...
                <Link 
                  key={link.name} 
                  href={link.href}
                  className={`${textClass} hover:text-primary-700 font-medium transition-colors`}
                >
                  {link.name}
                </Link>
              )
          ...
          <div className="md:hidden">
            <button
              ...
              className={`p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-opacity-50 ${textClass}`}
      ...
      {isMobileMenuOpen && (
        ...
        >
          <button onClick={() => setIsMobileMenuOpen(false)} aria-label="Sluiten" className="absolute inset-0 bg-gradient-to-br from-white/95 via-white/90 to-white/95 dark:from-gray-900/80 dark:via-gray-900/70 dark:to-gray-900/80 backdrop-blur-2xl" />
          <motion.div initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.25, ease: 'easeOut' }} className="relative h-full overflow-y-auto">
            <div className="container mx-auto px-4 pt-4 pb-28">
              <div className="flex items-center justify-between py-3">
                ...
                <button onClick={() => setIsMobileMenuOpen(false)} className="h-10 w-10 rounded-xl bg-white/70 dark:bg-gray-800/70 border border-gray-200/60 dark:border-gray-700/60 backdrop-blur flex items-center justify-center shadow-sm" aria-label="Sluiten">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-700 dark:text-gray-200" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>

              {mobileView !== 'root' && (
                <div className="mt-2 mb-1">
                  <button className="inline-flex items-center gap-2 text-sm text-gray-700 hover:text-primary-700" onClick={() => setMobileView('root')} aria-label="Terug">
                  ...
                  {navLinks.map((link) => (
                    link.hasDropdown ? (
                      <div key={link.name}>
                        ...
                        <button onClick={() => setMobileView('services')} className={`w-full flex items-center justify-between px-1 py-4 text-lg font-medium ${isActive ? 'text-primary-700 dark:text-primary-400' : 'text-gray-900 dark:text-gray-100'}`} aria-haspopup="menu" aria-controls="mobile-services">
                          <span>{link.name}</span>
                      ...
                      <div key={link.name}>
                        {(() => { const isActive = link.href === '/' ? pathname === '/' : pathname?.startsWith(link.href || ''); return (
                          <Link href={link.href} className={`flex items-center justify-between px-1 py-4 text-lg font-medium transition-colors ${isActive ? 'text-primary-700 dark:text-primary-400' : 'text-gray-900 dark:text-gray-100 hover:text-primary-700'}`} onClick={() => setIsMobileMenuOpen(false)}>
                            <span>{link.name}</span>
                            {isActive && <span className="text-xs text-primary-600">Huidig</span>}
                          </Link>
                        ); })()}
                        ...
                        <div className="flex flex-col">
                          {category.items.map((item) => (
                            (() => { const isActive = pathname?.startsWith(item.href || ''); return (
                              <Link key={item.name} href={item.href} className={`py-3 text-base flex items-center justify-between ${isActive ? 'text-primary-700 dark:text-primary-400' : 'text-gray-900 dark:text-gray-100 hover:text-primary-700'}`} onClick={() => setIsMobileMenuOpen(false)}>
                                <span>{item.name}</span>
                                {isActive && <span className="text-xs text-primary-600">Huidig</span>}
                              </Link>
                            ); })()
              ...
              {mobileView === 'root' && (
                <div className="mt-6">
                  ...
                  <div className="mt-3 flex gap-3">
                    <Link href="tel:+31636164419" onClick={() => setIsMobileMenuOpen(false)} className="flex-1 inline-flex items-center justify-center h-10 rounded-full bg-primary-600 text-white font-medium hover:bg-primary-700 transition-colors">Bel ons</Link>
                    <Link href="mailto:info@next-win.nl" onClick={() => setIsMobileMenuOpen(false)} className="flex-1 inline-flex items-center justify-center h-10 rounded-full border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 font-medium hover:border-primary-600 transition-colors">E-mail</Link>
                  </div>
                </div>
              )}
